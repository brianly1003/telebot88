  ...
  testClipboard: function (el) {
    Telegram.WebApp.readTextFromClipboard(function (clipText) {
      if (clipText === null) {
        $(el)
          .next("span")
          .html("Clipboard text unavailable.")
          .attr("class", "err");
      } else {
        $(el)
          .next("span")
          .text("(Read from clipboard: Â«" + clipText + "Â»)")
          .attr("class", "ok");
      }
    });
    return false;
  ...
  requestWriteAccess: function (el) {
    Telegram.WebApp.requestWriteAccess(function (allowed) {
      if (allowed) {
        ...
        $(el)
          .next("span")
          .html("(User declined this request)")
          .attr("class", "err");
      }
    });
  },
  requestPhoneNumber: function (el) {
    Telegram.WebApp.requestContact(function (sent) {
      if (sent) {
        $(el)
          .next("span")
          .text("(Phone number sent to the bot)")
          .attr("class", "ok");
      } else {
        $(el)
          .next("span")
