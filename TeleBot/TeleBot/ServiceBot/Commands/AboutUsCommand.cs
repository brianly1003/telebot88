...
{
    public class AboutUsCommand : BaseCommand
    {
        public override string Name => CommandNames.AboutUsCommand;
        public override string CommandText => TextCommands.AboutUs;

        public AboutUsCommand(ICommandHandler commandHandler, IUserStateManager userStateManager) : base(commandHandler, userStateManager)
        {
        }
        ...
        public override async Task ExecuteAsync(ITelegramBotClient botClient, Message? message, CallbackQuery? callback, CancellationToken cancellationToken)
        {
            ...
            {
                InlineKeyboardButton.WithCallbackData("Cancel", TextCommands.Start),
            });
